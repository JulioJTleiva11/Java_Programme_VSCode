...
public class Complex implements Serializable {
	...
	public static Vector<Complex> readData (String fname) {
		...
		try {
			...
			do {
				try {
					...
					if (next !=null && next instanceof Complex) {
						result.add((Complex) next);
					} else {
						stop = true;
					}
	...
	public static void writeData(int number) {
		...
		try {
			...
			for (int i=0; i<number; i++) {
				Complex next = new Complex(rnd.nextInt(100),rnd.nextInt(100));
	...
	public static void main(String[] args) {
		...
		if (write) {
			...
			// read written data
			Vector<Complex> data = Complex.readData(filename);
